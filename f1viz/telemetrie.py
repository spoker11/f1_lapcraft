...
def show_telemetrie():
    ...
    with st.form("telemetrie_form"):
        ...
        gp_name = st.selectbox("Velká cena", gp_names, key="tele_gp")
    ...
    params = st.session_state.get("telemetry_params")
    if not params:
        st.info("Vyber všechna pole a načti data.")
        return

    with st.spinner("Načítám session a seznam jezdců…"):
        schedule = get_schedule(params["year"])
        session, err = get_session(params["year"], params["gp_name"], params["session_type"], schedule)
    ...
    with col2:
        other_drivers = [d for d in drivers if d != driver1]
        # Pokud je jen jeden jezdec, vyplň driver2 stejně, ale disable
        driver2 = st.selectbox("Druhý jezdec", other_drivers if other_drivers else ["-"], key="tele_driver2", disabled=(len(other_drivers) == 0))

    # Pokud je jen jeden jezdec, dál nemá smysl pokračovat
    if not other_drivers:
        st.info("Není k dispozici druhý jezdec pro porovnání.")
        return

    laps1 = session.laps.pick_driver(driver1) if driver1 in drivers else None
    laps2 = session.laps.pick_driver(driver2) if driver2 in drivers else None

    lap_nums1 = laps1["LapNumber"].astype(int).tolist() if laps1 is not None and not laps1.empty else []
    lap_nums2 = laps2["LapNumber"].astype(int).tolist() if laps2 is not None and not laps2.empty else []
    ...
    with st.spinner("Načítám telemetrii a připravuji graf…"):
        try:
            lap1_row = laps1[laps1["LapNumber"] == lap1]
            lap2_row = laps2[laps2["LapNumber"] == lap2]
            if lap1_row.empty or lap2_row.empty:
                st.warning("Vybrané kolo není k dispozici pro vybraného jezdce.")
                return
            lap1_row = lap1_row.iloc[0]
            lap2_row = lap2_row.iloc[0]
            ...
            st.error(f"Chyba při vykreslení telemetrie: {e}")
